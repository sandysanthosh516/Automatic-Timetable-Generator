...
class Main {
    public static void main(String[] args) throws IOException {
        ...
        JComboBox<String> cr = new JComboBox<>(credits);
        JComboBox<String> cb = new JComboBox<>(Timet);
        JComboBox<String> pf = new JComboBox<>(profs);
        JComboBox<String> co = new JComboBox<>(course);
        ...
        JComboBox<String> d3 = new JComboBox<>(course);
        ...
        JComboBox<String> d7 = new JComboBox<>(profs);
        ...
        JComboBox<String> d11 = new JComboBox<>(profs);
        ...
        JComboBox<String> cb7 = new JComboBox<>(profs);
                ...
                if (cb4.getSelectedIndex() != 0 && cb7.getSelectedIndex() != 0) {
                    if (B.get(cb2.getText()).add(CO.get(cb4.getItemAt(cb4.getSelectedIndex()).toString()),
                        ...
                        JOptionPane.showMessageDialog(s,
                                "A new course has been added to the batch " + cb2.getText() + "\n" + "Course: "
                                        + cb4.getItemAt(cb4.getSelectedIndex()).toString() + "\n" + "Professor: "
                                        + cb7.getItemAt(cb7.getSelectedIndex()).toString());
                    } else {
                        JOptionPane.showMessageDialog(s,
                                "Periods are insufficient for adding this course to the batch");
                    }
                } else {
        ...
        JComboBox<String> cp4 = new JComboBox<>(course);
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    if (cp4.getSelectedIndex() != 0) {
                        ...
                        JOptionPane.showMessageDialog(s,
                                "The course " + cp4.getItemAt(cp4.getSelectedIndex()).toString()
                                        + " has been assigned to " + cp2.getText());
                    } else {
                        JOptionPane.showMessageDialog(s, "Select a valid course!");
                    }
                } catch (NullPointerException ex) {
                    JOptionPane.showMessageDialog(s, "Provide a valid details");
                }
            }
        });
        ...
        JComboBox<String> cc4 = new JComboBox<>(credits);
        ...
        d4.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                String courseName = (String) d3.getSelectedItem();
                if (courseName != null && !courseName.equals("Select")) {
                    // Remove course from the map
                    CO.remove(courseName);
                    // Remove course from combo boxes
                    d3.removeItem(courseName);
                    cb4.removeItem(courseName);
                    cp4.removeItem(courseName);
                    JOptionPane.showMessageDialog(s, "Course " + courseName + " has been deleted.");
                } else {
                    JOptionPane.showMessageDialog(s, "Select a valid course to delete.");
                }
            }
        });
        ...
        d8.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                String professorName = (String) d7.getSelectedItem();
                if (professorName != null && !professorName.equals("Select")) {
                    // Remove professor from the map
                    PR.remove(professorName);

                    // Remove professor from all ComboBoxes
                    d7.removeItem(professorName);
                    cb7.removeItem(professorName);
                    pf.removeItem(professorName);

                    // Reset timetable if the deleted professor was selected
                    if (pf.getSelectedItem() != null && pf.getSelectedItem().toString().equals(professorName)) {
                        for (int i = 0; i < 6; i++) {
                            for (int j = 0; j < 8; j++) {
                                timet[i][j + 1] = ""; // Clear timetable cells
                            }
                        }
                        view.revalidate();
                        view.repaint();
                    ...
                    JOptionPane.showMessageDialog(s, "Professor " + professorName + " has been deleted.");
                } else {
                    JOptionPane.showMessageDialog(s, "Select a valid professor to delete.");
                }
            }
        });
        ...
        d12.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                String batchName = (String) d11.getSelectedItem();
                if (batchName != null && !batchName.equals("Select")) {
                    // Remove batch from the map
                    B.remove(batchName);
                    // Remove batch from combo boxes
                    d11.removeItem(batchName);
                    cb.removeItem(batchName);
                    JOptionPane.showMessageDialog(s, "Batch " + batchName + " has been deleted.");
                } else {
                    JOptionPane.showMessageDialog(s, "Select a valid batch to delete.");
                }
            }
        });
